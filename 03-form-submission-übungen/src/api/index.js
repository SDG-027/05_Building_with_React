...
const contactFormSchema = z.object({
  firstName: z
    .string('First name needs to be a string')
    .min(1, 'First name is required')
    .max(512, 'Name too long'),
  lastName: z
    .string('Last name needs to be a string')
    .min(1, 'Last name is required'),
  email: z.email('Invalid email address'),
  message: z
    .string('Message needs to be a string')
...
export const registerNewsletter = async (email) => {
  ...
  const newsletterList =
    JSON.parse(localStorage.getItem('newsletterList')) || [];
  if (newsletterList.includes(data))
    throw new Error(`Email ${data} is already registered.`);
...
export const searchProducts = async ({
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesQuery =
      !data.query ||
      product.title.toLowerCase().includes(data.query.toLowerCase());

    return (
      matchesCategory && matchesMinPrice && matchesMaxPrice && matchesQuery
    );
  });
